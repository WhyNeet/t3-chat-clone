...
export const useSidebarStore = create<{
  ...
  isOpen: Boolean(Number(localStorage.getItem("sidebar-open") ?? 1)),
  toggle: () => {
    localStorage.setItem("sidebar-open", Number(!get().isOpen).toString());
...
export function ChatsSidebar() {
  ...
  return (
    <aside className="min-w-72 max-w-72 rounded-tr-3xl h-[calc(100vh-0.25rem)] absolute right-0 flex flex-col">
      ...
      {user && !isLoading ? (
        <NavLink to="/" className="mb-2 mx-3">
              ...
              <div className="flex flex-col-reverse">
                {Object.values(chats)
                  ...
                  .map(({ chat, streaming }) => (
                    ...
                    >
                      <div className="whitespace-nowrap max-w-10/12 overflow-hidden overflow-ellipsis">
                        {chat?.name ?? "New Chat"}
                      </div>
      ...
      {isLoggedIn ? (
        <div className="flex items-center justify-between rounded-full border border-pink-900/20 p-2 absolute bottom-2 inset-x-3 bg-pink-200/20 backdrop-blur-xl">
          <NavLink to="/settings">
